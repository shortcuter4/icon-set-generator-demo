...
-- ============================================
-- FUNCTION: generate_icon_set_from_tags
-- ============================================
...
v_set_id BIGINT;
    ...
    v_existing_bitmap roaringbitmap;
    v_lock_key BIGINT;
BEGIN
    -- Acquire advisory lock based on tag IDs
    v_lock_key := ('x' || substring(md5(array_to_string(p_tag_ids, ',')), 1, 15))::bit(60)::bigint;
    PERFORM pg_advisory_xact_lock(v_lock_key);

    -- Select random candidate icons
...
END IF;
    ...
    -- Optimized overlap check: only relevant sets
    v_candidate_bitmap := rb_from_array(v_sorted_ids);

SELECT rb_union_agg(set_ids_bitmap)
INTO v_existing_bitmap
FROM icon_set_items
WHERE icon_id = ANY(v_sorted_ids);

v_existing_bitmap := COALESCE(v_existing_bitmap, rb_from_array(ARRAY[]::BIGINT[]));

    IF rb_and_cardinality(v_candidate_bitmap, v_existing_bitmap) > CEIL(array_length(v_sorted_ids, 1) * p_overlap_threshold) THEN
        RETURN NULL;
END IF;
